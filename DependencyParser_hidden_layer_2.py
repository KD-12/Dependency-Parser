...
class DependencyParserModel(object):
    ...
    def build_graph(self, graph, embedding_array, Config):
        ...
        with graph.as_default():
            ...
            ###### Same number of nuerons in both hidden layers , therefore same Config.hidden_size used
            ...
            weights_input_1 = tf.Variable(tf.truncated_normal([Config.embedding_size*Config.n_Tokens,Config.hidden_size ],stddev=1.0 / math.sqrt(Config.embedding_size*Config.n_Tokens)))
            weights_input_2 = tf.Variable(tf.truncated_normal([Config.hidden_size,Config.hidden_size_2 ],stddev=1.0 / math.sqrt(Config.hidden_size_2)))
            biases_input_1 = tf.Variable(tf.zeros([Config.hidden_size]))
            biases_input_2 = tf.Variable(tf.zeros([Config.hidden_size_2]))
            weights_output = tf.Variable(tf.truncated_normal([Config.hidden_size_2,numTrans],stddev=1.0 / math.sqrt(Config.hidden_size)))

            # forwar pass for layer one the output of which is input to the second forward pass.
            y1 = self.forward_pass(embed, weights_input_1, biases_input_1, weights_input_2)


            self.prediction = self.forward_pass(y1, weights_input_2, biases_input_2, weights_output)
            ...
            ###### testing phase for NN with 2 layer #####################
            # For test data, we only need to get its prediction
            ...
            self.test_pred = self.forward_pass(test_y1, weights_input_2, biases_input_2, weights_output)
    ...
    def train(self, sess, num_steps):
        ...
        """
        self.init.run()
        print "Initailized Training for NN woth HH 2"
