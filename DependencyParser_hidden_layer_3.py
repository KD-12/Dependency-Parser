...
class DependencyParserModel(object):
    ...
    def build_graph(self, graph, embedding_array, Config):
        ...
        with graph.as_default():
            ...
            ### I have kept number of neurons in all hidden layers same thats why only one Config.hidden_size used.
            # initialising three input weights for three layers and three biases for three layers and one output weight in truncated range
            #and inside tf.variable so that they can be learnt

            weights_input_1 = tf.Variable(tf.truncated_normal([Config.embedding_size*Config.n_Tokens,Config.hidden_size ],stddev=1.0 / math.sqrt(Config.hidden_size)))
            weights_input_2 = tf.Variable(tf.truncated_normal([Config.hidden_size,Config.hidden_size ],stddev=1.0 / math.sqrt(Config.hidden_size)))
            weights_input_3 = tf.Variable(tf.truncated_normal([Config.hidden_size,Config.hidden_size ],stddev=1.0 / math.sqrt(Config.hidden_size)))
            ...
            #forward pass for layer one
            y1 = self.forward_pass(embed, weights_input_1, biases_input_1, weights_input_2)
            # forward pass for layer two
            y2 = self.forward_pass(y1, weights_input_2, biases_input_2, weights_input_3)
            # forward pass for layer three to get predictions.
            self.prediction = self.forward_pass(y2, weights_input_3, biases_input_3, weights_output)

            #after_dropout_layer = tf.nn.dropout(after_mask, self.prob)

            # cross entropy loss using above predictions and the labels.
            ...
            # l2 regularizing for three input weights of three layers & one output weight and three biases of three layers.
            ip_weight_1_regularizer = tf.nn.l2_loss(weights_input_1)
            ip_weight_2_regularizer = tf.nn.l2_loss(weights_input_2)
            ip_weight_3_regularizer = tf.nn.l2_loss(weights_input_3)
            h_weight_regularizer = tf.nn.l2_loss(weights_output)
            h_bias_1_regularizer = tf.nn.l2_loss(biases_input_1)
            h_bias_2_regularizer = tf.nn.l2_loss(biases_input_2)
            h_bias_3_regularizer = tf.nn.l2_loss(biases_input_3)
            embed_regularizer  = tf.nn.l2_loss(embed)

            # loss by addind the cross entropy loss with l2_penalties
            self.loss = tf.reduce_mean(ce_loss + (Config.lam) *  ip_weight_1_regularizer
                                               + (Config.lam) *  h_weight_regularizer
                                               + (Config.lam) *  h_bias_1_regularizer
                                               + (Config.lam) *  ip_weight_2_regularizer
                                               + (Config.lam) *  ip_weight_3_regularizer
                                               + (Config.lam) *  h_bias_2_regularizer
                                               + (Config.lam) *  h_bias_3_regularizer
            ...
            ################################ Without Using Clippped Gradients ######################################
            #self.app = optimizer.apply_gradients(grads)

            # For test data, we only need to get its prediction


            ###### testing phase for NN with 3 layer #####################
            # For test data, we only need to get its prediction

            test_embed = tf.nn.embedding_lookup(self.embeddings, self.test_inputs)
            test_embed = tf.reshape(test_embed, [1, -1])
            test_y1 = self.forward_pass(test_embed, weights_input_1, biases_input_1, weights_input_2)

            test_y2 = self.forward_pass(test_y1, weights_input_2, biases_input_2, weights_input_3)

            self.test_pred = self.forward_pass(test_y2, weights_input_3, biases_input_3, weights_output)
    ...
    def train(self, sess, num_steps):
        ...
        """
        self.init.run()
        print "Initailized Training for NN woth HH 3"
